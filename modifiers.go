...
func addReverbToAudio(channel string) {
	...
	cmd := exec.Command("ffmpeg", "-i", "input-"+channel+".mp3", "-i", "static/reverb.wav", "-filter_complex", "[0:a]apad=pad_dur=2,aformat=channel_layouts=stereo[dry];[0:a]apad=pad_dur=2,aformat=channel_layouts=stereo,afir=dry=10:wet=10[wet];[dry][wet]amix=weights='0.9 0.1',loudnorm=I=-36:TP=-1.5:LRA=11", "-b:a", "320k", "output-"+channel+".mp3")
	err := cmd.Run()
	if err != nil {
